...
@if ($item->assentCount > 10)
    <p class="bd_like_more_text">{!! xe_trans('board::assentThisPosts', [
        'count'=> sprintf('<a href="#" data-toggle="xe-page-modal" data-url="%s" data-params="{}" data-callback="getVotedUserList">%s</a>', $urlHandler->get('votedModal', ['option' => $option, 'id' => $item->id]), $item->assentCount - 1),
    ]) !!}</p>
@endif