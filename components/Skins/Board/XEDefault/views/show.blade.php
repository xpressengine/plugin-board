                ...
                <div class="xf-post-edit-box">
                    <ul class="xf-list xf-edit-list">
                        ...
                        @if($isManager === true || $item->user_id === Auth::user()->getId() || $item->user_type === $item::USER_TYPE_GUEST)
                            <li class="xf-edit-item">
                                <a href="{{ $urlHandler->get('edit', array_merge(Request::all(), ['id' => $item->id])) }}" class="xf-edit__link xf-a"> {{ xe_trans('xe::modify') }}</a>
                            </li>
                            <li class="xf-edit-item">
                                <a href="#" data-url="{{ $urlHandler->get('destroy', array_merge(Request::all(), ['id' => $item->id])) }}" class="xf-edit__link xf-a bd_delete">{{ xe_trans('xe::delete') }}</a>
                            </li>
                        @endif
                    </ul>
                </div>
